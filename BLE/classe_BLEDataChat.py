...
from classe_EcranOLED import EcranOLED
...
class BLEDataChat:
    def __init__(self,name = ""):
        ...
        # Debug/Traces
        self._tft = EcranOLED("Pico " + name)
        self._log("Init BLE")

        # Status
        self._default_data_to_send = 0x42
        self._default_data_to_send_increment = 0x1
        self._data_received = 0x0
        self._new_data_received = 0
    ...
    def _log(self, msg):
        #print(msg)
        #self._tft.DisplayText(msg + "\n")
        self._tft.DisplayLog(msg)
        
    def _advertise(self, interval_us=500000):
        self._ble.gap_advertise(interval_us, adv_data=self._payload)
        #self._log("Publicite BLE active")
        #self._log("Fct advertise")
        
    def _irq(self, event, data):
        if event == _IRQ_CENTRAL_CONNECT:
            conn_handle, _, _ = data
            self._connections.add(conn_handle)
            self._log("Central Connected")

        elif event == _IRQ_CENTRAL_DISCONNECT:
            conn_handle, _, _ = data
            self._connections.discard(conn_handle)
            self._log("Central Disconnected")
            self._advertise()

        elif event == _IRQ_SCAN_RESULT:
            addr_type, addr, adv_type, rssi, adv_data = data
            if DATA_SERVICE_UUID in decode_services(adv_data):
                self._log("Periph Found")
            ...
            self._log("Periph Connected")
            self._ble.gattc_discover_services(conn_handle)

        elif event == _IRQ_PERIPHERAL_DISCONNECT:
            self._log("Periph Disconnected")
            self._peer_conn = None
            self._ble.gap_scan(2000, 30000, 30000)
            ...
            if self._peer_conn:
                self._ble.gattc_discover_characteristics(self._peer_conn, self._start_handle, self._end_handle)
            ...
            if self._peer_conn and self._peer_value_handle:
                #self._log("Ready to Rx")
                # On active la notification
                self._ble.gattc_write(self._peer_conn,self._peer_value_handle + 1,struct.pack("h", 1),1)

        elif event == _IRQ_GATTC_NOTIFY:
            conn_handle, value_handle, notify_data = data
            value = struct.unpack("<h", notify_data)[0]
            self._data_received = hex(value)
            if (self._data_received == hex(0x11)):
                self._new_data_received = 1
            self._log(f"Receive: 0x{value:X}")
            '''
            try:
                ...
                self._log("Erreur reception")
                #value = struct.unpack("<h", notify_data)[0]
                #self._log(f"Recu: 0x{value:X}")
            '''
        
    def send_default_data(self):
        value = self._default_data_to_send + self._default_data_to_send_increment
        self._default_data_to_send_increment += 0x1
        data = struct.pack("<h", value)
        self._ble.gatts_write(self._data_handle, data)
        self._log(f"Send: 0x{value:X}")
        for conn_handle in self._connections:
            self._ble.gatts_notify(conn_handle, self._data_handle)

    def send_data(self,user_data):
        data = struct.pack("<h", user_data)
        self._ble.gatts_write(self._data_handle, data)
        self._log(f"Send: 0x{user_data:X}")
        for conn_handle in self._connections:
            self._ble.gatts_notify(conn_handle, self._data_handle)
    ...
    def is_new_data_received(self):
        return(self._new_data_received)
    
    def clear_new_data_received(self):
        self._new_data_received = 0
...
def main():
    ...
    while True:
        ...
        ble_chat.send_text("Hello")
        time.sleep(1)
        ble_chat.send_default_data()
        time.sleep(1)
        ble_chat.send_data(50)
        time.sleep(1)
    '''    
    while True:
        try:
